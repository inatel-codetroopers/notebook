...
// retorna 1 se R esta a esquerda do vetor P->Q, -1 se estiver a direita. 0 se P, Q e R forem colineares
int ccw(point p, point q, point r){
	return cmp(cross(q - p, r - p));
}

// Rotaciona um ponto em relacao a origem em 90 graus sentido anti-horario
point RotateCCW90(point p)	{ return point(-p.y, p.x); }
// Rotaciona um ponto em relacao a origem em 90 graus sentido horario
point RotateCW90(point p)	{ return point(p.y, -p.x); }

// Rotaciona um ponto P em A graus no sentido anti-horario em relacao a origem; Para rotacionar no sentido horario, basta A ser negativo
...
/* Distancia de ponto P para reta que passa por [A,B]. Armazena em C (por ref) o ponto projecao de P na reta. */
...
/* Distancia de ponto P ao segmento [A,B]. Armazena em C (por ref) o ponto de projecao de P em [A,B]. Se este ponto estiver fora do segmento, eh retornado o mais proximo. */
double distancePointSeg(point p, point a, point b, point& c){
	if ((b-a)*(p-a) <= 0) { c = a; return abs(a-p); }
	if ((a-b)*(p-b) <= 0) { c = b; return abs(b-p); }
...
/* Encontra a interseccao das retas (p-q) e (r-s) assumindo que existe apenas 1 intereccao. Se for entre segmentos, verificar se interseptam primeiro. */
...
/*Dado dois pontos (A, B) de uma circunferencia e seu raio R, eh possivel obter seus possiveis centros (C1 e C2). Para obter o outro centro, basta inverter os paramentros */
...
double dot(const cxpt &a, const cxpt &b) {
...
double circ_inter_area(circle &a, circle &b) {
	...
	return a.r*a.r*(A-0.5*sin(2*A))+b.r*b.r*(B-0.5*sin(2*B));
}
