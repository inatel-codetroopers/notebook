...
(1)Se for preciso manter pontos colineares na borda do convex hull, essa parte evita que eles sejam removidos;
*/
...
vector<point> convexhull(vector<point> &T){
	...
	/*(1)*/int k; for(k=n-2; k>=0 && ccw(T[0],T[n-1],T[k])==0; k--);
	reverse((k+1)+all(T)); /*(1)*/
