/*Poligono eh representado como um array de pontos T[i] sao os vertices do poligono. Existe uma aresta que conecta T[i] com T[i+1], e T[size-1] com T[0]. Logo assume-se que T[0] != T[size-1]
Poligono simples: Aquele em que as arestas nao se interceptam. Convexo: O angulo interno de T[i] com T[i-1] e T[i+1] <= 180. Concavo: Existe algum i que nao satisfaz a condicao anterior*/

/* Retorna a area com sinal de um poligono T. Se area > 0, T esta listado na ordem CCW */
...
/* Retorna a area de um poligono T. (pode ser concavo ou convexo) em O(N)*/
...
/* Retorna o perimetro do poligono T. (pode n funcionar como esperado se o poligono for uma linha reta (caso degenerado))*/
double poly_perimeter(polygon& T){
	double perimeter = 0;
	int n = T.size();
	if (n < 3) return 0;
	rep(i, 0, n)
		perimeter += abs(T[i] - T[(i+1)%n]);
	return perimeter;
}
...
bool isConvex(polygon& T){
	...
	if (giro == 0)	return false; //todos pontos sao colineares

	rep(i, 0, n){
		int t = ccw(T[i], T[(i+1)%n], T[(i+2)%n]);
		if (t != 0 && t != giro) return false;
	}
	return true;
}
...
int in_poly(point p, polygon& T){
	...
	rep(i, 0, N){
		if (between(p, T[i], T[(i+1)%N])) return -1;
		a += arg(T[i], p, T[(i+1)%N]);
	}
	return cmp(a) != 0;
}
...
polygon poly_intersect(polygon& P, polygon& Q){
	...
	while ((aa<n || ba<m) && aa<2*n && ba<2*m){
		...
		if (cross==0 && ccw(p1, q1, p2)==0 && cmp(dot(A,B))<0){
			if (between(q1, p1, p2)) R.push_back(q1);
			if (between(q2, p1, p2)) R.push_back(q2);
			if (between(p1, q1, q2)) R.push_back(p1);
			if (between(p2, q1, q2)) R.push_back(p2);
